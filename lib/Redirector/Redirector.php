	...
	class Redirector extends CM_Module
	{
		...
		public function AddRule($name, RedirectorRule $rule)
		{
			if ($rule !== null && !empty($name)) {
				$rule->SetRedirector($this);
				
				$rule->beforeAddToRedirector($this);
		...
		{
			if (!empty($this->_arRules)) {
				foreach ($this->_arRules as $rule) {
					$arNewAdminMenu = $rule->getAdminMenu();
					
					if (!empty($arNewAdminMenu)) {
						AdminMenu::addMenuItem($arNewAdminMenu);
					}
				}
			}
			
			return parent::getAdminMenu(); // TODO: Change the autogenerated stub
		}
	}
